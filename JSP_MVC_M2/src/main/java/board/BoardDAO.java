...
public class BoardDAO {
	...
	private final String BOARD_INSERT = 
		"insert into board (id, m_title, m_write, m_cont) "
	...
	private final String BOARD_UPDATE = "update board set m_title= ?, m_write= ? , m_cont = ? where id = ?"; 
	...
	private final String BOARD_LIST_T = "select * from board where m_title like '%'||?||'%' order by id desc"; 
	private final String BOARD_LIST_W = "select * from board where m_write like '%'||?||'%' order by id desc"; 
	private final String BOARD_LIST_C = "select * from board where m_cont like '%'||?||'%' order by id desc"; 
	private final String BOARD_LIST_R = "select * from board where regdate like '%'||?||'%' order by id desc"; 
	...
	public List<BoardDTO> getBoardList(BoardDTO dto) 
	{
		...
		try 
		{
			...
			// searchConditon 변수의 넘어오는 값에 따라서 분기 처리 
			if (dto.getSearchCondition().equals("M_TITLE")) 
			{
				pstmt = conn.prepareStatement(BOARD_LIST_T) ;				
			}
			
			else if (dto.getSearchCondition().equals("M_WRITE") ) 
			{
				pstmt = conn.prepareStatement(BOARD_LIST_W) ;
			}
			
			else if (dto.getSearchCondition().equals("M_CONT") ) 
			{
