...
public class MemberDAO 
{
	...
	"insert into member (seq, id, pw, email) "
	+ "values ((select nvl(max(seq),0), ? , ? , ?)";
		    
	// DB의 Member 테이블의 모든 레코드를 출력 쿼리 : 레코드가 여러개 : dto ==> ArrayList 리턴 
	private final String MEMBERS_LIST = "select * from member" ; 
			
	// DB의 Member 테이블의 글 상세 조회 : 레코드 1개  <== dto
	private final String MEMBERS_GET = "select seq, id, email from member where id = ?" ; 
