...
public class FoodDAO 
{
	...
	// DB의 Food 테이블 조히 쿼리 
	private final String FOOD_GET = "select * from Food where foodn = ?"; 

	// DB의 Food 테이블의 전체 조회 쿼리 
	private final String FOOD_LIST = "select * from Food"; 
	...
	public void insertFood (FoodDTO dto) 
	{
		...
		try 
		{
			...
			//pstmt 객체의 ? 변수의 값 할당. 
			pstmt.setString(1, dto.getNation());
			pstmt.setString(2, dto.getFoodn());
			pstmt.setInt(3, dto.getPrice());
			
			//pstmt 를 실행해서 DB에 저장 
			pstmt.executeUpdate(); 
			
			System.out.println("Food 테이블에 값이 잘 insert 되었습니다. ");
	...
	public List<FoodDTO> getFoodList(FoodDTO dto) 
	{
		...
		try 
		{
			...
			//BOARD_LIST = "select * from from order by seq desc" 
			pstmt = conn.prepareStatement(FOOD_LIST) ; 
			...
			while (rs.next()) {
				...
				FoodDTO food = new FoodDTO();
				    // 루프 블락 내에 선언 
				food.setSeq(rs.getInt("SEQ"));
				food.setNation(rs.getString("NATION"));
				food.setFoodn(rs.getString("FOODN"));
	...
	public FoodDTO getFood(FoodDTO dto) 
	{
		...
		try 
		{
			...
			pstmt = conn.prepareStatement(FOOD_GET); 
			pstmt.setString(1, dto.getFoodn());
	...
	{
		System.out.println("updateFood 메소드 호출됨");
		
		try 
		{
			conn = util.getConnection(); 
			//BOARD_UPDATE = "update board set title= ?, write= ? , content = ? where seq = ?"
			pstmt = conn.prepareStatement(FOOD_UPDATE); 
	...
	public void deleteFood (FoodDTO dto) 
	{
		...
		{
			conn = util.getConnection(); 
			// FOOD_DELETE = "delete food where seq = ?"
			pstmt = conn.prepareStatement(FOOD_DELETE); 
